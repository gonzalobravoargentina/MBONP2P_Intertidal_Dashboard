...
# Crear el mapa
map <- leaflet(AMP) %>% addProviderTiles(providers$Esri.WorldImagery) %>%
...
# Obtener una lista única de localidades
unique_sites <- unique(AMP$Comments)

makeMyIcon <- function(label) {
  div <- paste0("<div class='my-custom-icon'>", label, "</div>")
  htmltools::HTML(div)
}

# Iterar sobre la lista única de localidades y agregar un botón de zoom para cada una
for (Comments in unique_sites) {
  # Obtener la primera fila de la localidad actual
  row <- AMP[AMP$Comments == Comments, ][1, ]
  
  # Crear el botón de zoom personalizado
  myIcon <- makeMyIcon(Comments)
  
  # Agregar el botón de zoom personalizado
  map <- map %>% 
    addEasyButton(
      easyButton(
        icon = myIcon,
        title = paste("Zoom a", Comments),
        position = "topright",
        onClick = JS(paste("function(btn, map) { map.flyTo([", 
                          as.numeric(row$Latitude), ", ", 
                          as.numeric(row$Longitude), "], 12); }"))
      )
    )
}
