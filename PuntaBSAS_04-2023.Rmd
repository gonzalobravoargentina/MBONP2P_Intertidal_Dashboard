...
AMP.cover <- read.csv("PtaBSAS_04-2023_percent_covers.csv")

AMP.metadata <- read.csv("PtaBSAS_04-2023_metadata.csv")
setwd("..")# original WD
...
library(readr)
setwd(paste0(getwd(),"/DATA"))#set new WD to folder DATA
SST = read_csv("PtaBSAS_04-2023_SST.csv")
SST.clim = read_csv("PtaBSAS_04-2023_Climatology.csv")
setwd("..")# original WD
```
...
# create a pie chart for a single stratum without textinfo
plot_ly(labels=~CATAMI, values=~cover.mean, legendgroup=~CATAMI) %>%
  add_pie(data =sel_data.M, name = "Medio",title = 'Mid tide', domain = list(row = 0, column = 1))%>%
  layout(showlegend = T,
         grid=list(rows=1, columns=3),
         xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
         yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE))
